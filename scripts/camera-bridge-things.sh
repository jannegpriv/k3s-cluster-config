...
$SSH "$HOST" 'export KUBECONFIG=$HOME/.kube/config; kubectl -n openhab exec openhab-production-0 -c openhab514 -- sh -c "pkill -f \"^/usr/bin/ffmpeg .*camera-bridge\" || true; for d in c720_lillstugan c220_lillstugan c425_landet_baksida c425_carport; do mkdir -p /dev/shm/ipcamera/\$d; rm -f /dev/shm/ipcamera/\$d/*; chown openhab:openhab /dev/shm/ipcamera/\$d; done"'
