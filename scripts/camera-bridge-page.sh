...
# Needs an openHAB API token: OPENHAB_TOKEN=oh.xxx scripts/camera-bridge-page.sh [base-url]
set -euo pipefail
cd "$(dirname "$0")/.."
BASE="${1:-http://192.168.50.75:30080}"
: "${OPENHAB_TOKEN:?set OPENHAB_TOKEN (Main UI > profile > Create new API token)}"
